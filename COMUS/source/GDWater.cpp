...
void CGDWater::InitSimulation()
{
	...
	if (m_ICELLHHPRN != 0)
	{
		...
		if (!m_HeadOutFile.Open(strOutFile, CFile::modeCreate | CFile::modeWrite))
		{
			printf(_T("\n"));
			printf(_T("打开<%s>文件失败!\n"), _T("CellHead.out"));
			PauAndQuit();
	...
	if (m_ICELLDDPRN != 0)
	{
		...
		if (!m_HDownOutFile.Open(strOutFile, CFile::modeCreate | CFile::modeWrite))
		{
			printf(_T("\n"));
			printf(_T("打开<%s>文件失败!\n"), _T("CellDD.out"));
			PauAndQuit();
	...
	if (m_ICELLFLPRN != 0)
	{
		...
		if (!m_FLowOutFile.Open(strOutFile, CFile::modeCreate | CFile::modeWrite))
		{
			printf(_T("\n"));
			printf(_T("打开<%s>文件失败!\n"), _T("CellFlow.out"));
			PauAndQuit();
	...
	if (m_ICELLBDPRN != 0)
	{
		...
		if (!m_CellBDOutFile.Open(strOutFile, CFile::modeCreate | CFile::modeWrite))
		{
			printf(_T("\n"));
			printf(_T("打开<%s>文件失败!\n"), _T("CellBD.out"));
			PauAndQuit();
...
void CGDWater::OutPutFlowTXT()
{
	...
	CString strText;
	double OutVal;
	CArchive ar(&m_FLowOutFile, CArchive::store, 2097152);
	for (size_t i = 0; i < 3; i++)
	{
		if (i == 0)
		{
			//右侧流量
			strText = _T(" FLOW RIGHT FACE");
		}
		else if (i == 1)
		{
			//前侧流量
			strText = _T(" FLOW FRONT FACE");
		}
		else
		{
			//下侧流量
			strText = _T(" FLOW LOWER FACE");
		}
		//写数据块注释信息
		//1.写入当前时段数
		if (m_ICELLFLPRN == 1)
...
void CGDWater::OutPutCellBDTXT()
{
	...
	CString strText;
	double OutVal;
	CArchive ar(&m_CellBDOutFile, CArchive::store, 2097152);
	for (size_t i = 0; i < CGridCell::m_ItemMeanAry.size() - 1; i++)
	{
		//注: 最后一项是网格单元的水量平衡误差, 不输出到文件
		strText = CGridCell::m_ItemMeanAry[i];
		//写数据块注释信息
		//1.写入当前时段数
		if (m_ICELLBDPRN == 1)
		{
